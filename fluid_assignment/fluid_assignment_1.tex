...
\noindent \textbf{Q2} For a Rankine half body, the stagnation point lies on the positive x-axis where $\theta = 0$. Substituting $\theta = 0$ into the stream function gives $\psi = 0$. Therefore, the streamline passing through the stagnation point corresponds to $\psi = 0$. The equation of the streamline passing through the stagnation point is given by:

\begin{equation*}
    \psi = 0 \implies Ur\sin\theta + \frac{m}{2\pi}\theta = 0
\end{equation*}
...
\begin{enumerate}
    ...
    \item [\textbf{(iv)}] The force I think drive this flow is the pressure difference between the tap and the basin. The water flows from the higher 0pressure region (tap) to the lower pressure region (basin) due to gravity and the pressure exerted by the water supply system.
